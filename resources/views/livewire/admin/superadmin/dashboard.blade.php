...
<div class="dark:bg-zinc-800">
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <x-card header="Conclaves" minimize class="dark:bg-zinc-900 dark:text-gray-200 text-zinc-900">
            <div class="text-5xl m-3 font-[montserat]">{{ $total_chapters }}</div>
        ...
        <x-card header="Members" minimize class="dark:bg-zinc-900 dark:text-gray-200 text-zinc-900">
            ...
            <span><small>Total members across all chapters</small></span>
        ...
        <x-card header="Teams" minimize class="dark:bg-zinc-900 dark:text-gray-200 text-zinc-900">
            ...
            <span><small>Active teams across all chapters</small></span>
            <x-link :href="route('admin.dashboard.teams')" text="View Teams"
                icon="arrow-up-right" position="right" wire:navigate />
        </x-card>

        <x-card header="Chapter Admins" minimize class="dark:bg-zinc-900 dark:text-gray-200 text-zinc-900">
            <div class="text-5xl m-3 font-[montserat]">{{ $total_admins }}</div>
            <span><small>Total chapter admins</small></span>
                    ...
                    <tbody>
                        @forelse ($recent_members as $member)
                            <tr class="border-t border-zinc-200 dark:border-zinc-700">
                                ...
                                <td class="py-2 pr-4 text-zinc-600 dark:text-zinc-400">{{ $member['chapter_name'] ?? 'N/A' }}</td>
    ...
    <div class="mt-6">
        <x-card header="Recent Chapters" class="dark:bg-zinc-900 dark:text-gray-200 text-zinc-900">
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    <thead class="text-left text-zinc-500 dark:text-zinc-400">
                        <tr>
                    ...
                    <tbody>
                        @forelse ($chapters_list as $chapter)
                            <tr class="border-t border-zinc-200 dark:border-zinc-700">
                                <td class="py-2 pr-4">{{ $chapter['name'] }}</td>
                                <td class="py-2 pr-4 text-zinc-600 dark:text-zinc-400">{{ $chapter['users_count'] }}</td>
                                <td class="py-2 pr-4 text-zinc-600 dark:text-zinc-400">{{ $chapter['teams_count'] }}</td>
                            </tr>
                        @empty
                            <tr class="border-t border-zinc-200 dark:border-zinc-700">
